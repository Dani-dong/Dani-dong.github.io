...
![대표 이미지](../images/seoul-real-estate-eda-thumbnail.png)
...
- ✅ **데이터 전처리**: 결측치 현황 시각화 파악 및 건축년도 '0년' 등 이상 데이터 제거
...
![차트 1](../images/seoul-real-estate-eda-chart1.png)

**👉 발견한 것**:  
결측치(Null) 시각화 차트를 통해 토지면적 등 빈칸 분포를 한눈에 파악했습니다. 특히 건축년도 변수에서 '0년'으로 잘못 기입된 이상 데이터(Outlier)를 발견했습니다.
...
![차트 2](../images/seoul-real-estate-eda-chart2.png)

**👉 발견한 것**:  
수치형 변수인 건물면적(㎡)과 물건금액(만원)의 관계를 산점도로 시각화하여, 면적이 넓어질수록 가격이 상승하는 전반적인 양의 상관관계를 확인했습니다.

**🔍 왜 그럴까?**:  
범주형 변수인 자치구명을 더하여 그룹별 박스플롯을 그려본 결과, 동일한 면적이라도 위치(자치구)에 따라 인프라와 수요 차이로 인해 금액 차이가 크게 발생하는 것을 확인했습니다.

---

### 📊 분석 3: 다변량 분석과 제3의 변수 발견

![차트 3](../images/seoul-real-estate-eda-chart3.png)

**👉 발견한 것**:  
건축년도와 물건금액의 산점도에서, 지은 지 오래된 노후 주택의 가격이 다시 비싸지는 U자형 패턴을 관찰했습니다.
...
- 결측치나 오류 데이터를 코드로만 찾기보다 missingno 등 시각화 도구로 눈으로 직접 확인했을 때 직관적인 파악이 가능함을 배웠습니다.
